...
.DESCRIPTION
    ...
    scans for browser data directories, and launches Hindsight for each detected profile.

    Execution is asynchronous ("fire-and-forget") to avoid RTR timeouts.
    Artifact verification is handled by the downstream collection script.

.PARAMETER json_input
    A JSON string containing configuration parameters.
    Required: forensic_analysis_path, forensic_tool_path, output_format, selected_browser, system_hostname
    Optional: target_username

.OUTPUTS
    JSON string containing processing results and expected artifact paths.
#>
...
$target_username        = $json_data.target_username
...
try {
    if (-not [string]::IsNullOrWhiteSpace($target_username)) {
        $target_user_object = Get-CimInstance Win32_UserProfile | Where-Object {
        ...
        $current_user = (Get-CimInstance Win32_ComputerSystem).UserName -replace '^.+\\', ''
        $user_profile_path = (Get-CimInstance Win32_UserProfile | Where-Object {
            $_.LocalPath -like "*\$current_user"
        }).LocalPath
    }
